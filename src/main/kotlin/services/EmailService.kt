...
/*class EmailService {
    ...
    private val smtpPassword = System.getenv("BREVO_SMTP_KEY")
        ?: throw IllegalStateException("BREVO_SMTP_KEY is not set")
    ...
    private val properties = Properties().apply {
        ...
        put("mail.smtp.port", "587")
        put("mail.smtp.ssl.trust", "smtp-relay.brevo.com")
    }
    ...
    suspend fun sendOtpEmail(recipientEmail: String, otpCode: String): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val message = MimeMessage(session).apply {
                    setFrom(InternetAddress("noreply@yourdomain.com", "Your App Name"))
                    setRecipients(Message.RecipientType.TO, InternetAddress.parse(recipientEmail))
                    subject = "Your Login OTP Code"
                        ...
                        <html>
                            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                                <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                                    ...
                                    <p>Hello,</p>
                                    <p>Use the above 6-digit code to complete your login. The code expires in <strong>10 minutes</strong>.</p>
