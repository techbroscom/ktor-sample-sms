...
class UserPermissionService(
    ...
    suspend fun assignPermissionsToUser(
        ...
        return userPermissionsRepository.assignPermissions(userUUID, tenantId, request.featureIds, grantedByUUID)
    }

    suspend fun getUserPermissions(tenantId: String, userId: String): List<UserPermissionDto> {
        val userUUID = try {
            UUID.fromString(userId)
        } catch (e: IllegalArgumentException) {
            throw ApiException("Invalid userId format", HttpStatusCode.BadRequest)
        ...
        return userPermissionsRepository.findByUser(userUUID, tenantId, includeFeature = true)
    }

    suspend fun getEnabledFeatureKeys(tenantId: String, userId: String): List<String> {
        val userUUID = try {
            UUID.fromString(userId)
        } catch (e: IllegalArgumentException) {
            throw ApiException("Invalid userId format", HttpStatusCode.BadRequest)
        ...
        return userPermissionsRepository.getEnabledFeatureKeys(userUUID, tenantId)
    }
    ...
    suspend fun updatePermission(
        ...
        val updated = userPermissionsRepository.update(userUUID, tenantId, featureId, request)
        if (!updated) {
            throw ApiException("Permission not found", HttpStatusCode.NotFound)
        }

        return userPermissionsRepository.findByUserAndFeature(userUUID, tenantId, featureId)
            ?: throw ApiException("Permission not found after update", HttpStatusCode.InternalServerError)
    }

    suspend fun removePermission(tenantId: String, userId: String, featureId: Int): Boolean {
        val userUUID = try {
            UUID.fromString(userId)
        } catch (e: IllegalArgumentException) {
            throw ApiException("Invalid userId format", HttpStatusCode.BadRequest)
        ...
        val deleted = userPermissionsRepository.delete(userUUID, tenantId, featureId)
        if (!deleted) {
            throw ApiException("Permission not found", HttpStatusCode.NotFound)
        }
    ...
    suspend fun removeAllPermissionsForUser(tenantId: String, userId: String): Boolean {
        val userUUID = try {
            UUID.fromString(userId)
        } catch (e: IllegalArgumentException) {
            throw ApiException("Invalid userId format", HttpStatusCode.BadRequest)
        ...
        return userPermissionsRepository.deleteAllForUser(userUUID, tenantId)
    }
}
